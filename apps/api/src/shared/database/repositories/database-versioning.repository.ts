...
> implements DatabaseVersioningInterfaceRepository<T> {
  ...
  protected getIdColumn(): string {
    return (
      this.getMetadata().primaryColumns.find((col) => col.propertyName !== 'version')
        ?.propertyName || 'id'
    );
  }
  ...
  public async getTotalCountLatest(options: FindOneOptions<T> = {}): Promise<number> {
  ...
  public async findOneByVersion(id: string | number, version: number): Promise<T | null> {
    const where: FindOptionsWhere<T> = {
      [this.getIdColumn()]: id,
  ...
  public async findAllVersions(id: string | number, options?: FindManyOptions<T>): Promise<T[]> {
  ...
  public async updateAssociations<U extends { id: number | string } & Record<string, unknown>>({
    existingItems,
    updatedItems,
    keys,
    onCreate,
