...
> implements DatabaseInterfaceRepository<T> {
  ...
  public getRelationSearchFields(relationPath: string): string[] {
    const relation = this.getMetadata().findRelationWithPropertyPath(relationPath);
  ...
  public async getTotalCount(options: FindOneOptions<T> = {}): Promise<number> {
    const normalizedWhere = normalizeWhereForTypeOrm(options.where);
    ...
    for (const existing of existingItems) {
      ...
      if (updated) {
        ...
        for (const key of Object.keys(updated) as (keyof U)[]) {
          if (key !== 'id' && key !== key1 && key !== key2 && updated[key] !== existing[key]) {
