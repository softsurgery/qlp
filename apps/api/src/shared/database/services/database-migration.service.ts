...
export class MigrationService {
  ...
  async findAll(query: IQueryObject): Promise<MigrationEntity[]> {
    const queryBuilder = new QueryBuilder(this.migrationRepository.getMetadata());
    const queryOptions = queryBuilder.build(query);
    return await this.migrationRepository.findAll(queryOptions as FindManyOptions<MigrationEntity>);
  }
  ...
  private parse(filename: string, content: string): MigrationFile {
    const PATTERN = /^V(?<major>\d+)(_?(?<minor>\d+))?__(?<description>[a-zA-Z_-]+)\.sql$/;
    const RPATTERN = /^R(?<major>\d+)(_?(?<minor>\d+))?__(?<description>[a-zA-Z_-]+)\.sql$/;
  ...
  async runMigrations(migrationPath: string, migrationFiles: MigrationFile[]) {
    ...
    for (const migrationFile of migrationFilesSorted) {
      ...
      if (existingMigration) {
        if (!existingMigration.success && migrationFile.checksum !== existingMigration.checksum) {
          throw new MigrationChecksumValidationException(migrationFile.description);
        }
  ...
  private async executeMigration(migrationPath: string, migrationFile: MigrationFile) {
    const queries = readFileSync(`${migrationPath}/${migrationFile.script}`, 'utf-8')
      .split(';')
      .map((line) => line.trim());
    for (const query of queries) if (query) await this.dataSource.query(query);

    this.logger.log(`Executed migration: ${migrationFile.version} ${migrationFile.description}`);
  }
