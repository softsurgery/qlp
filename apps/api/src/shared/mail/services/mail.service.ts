...
export class MailService {
  ...
  async renderTemplateFromDb(
    ...
    if (!template?.content) {
      throw new Error(`Template "${templateName}" not found or has no EJS content.`);
    }

    const renderedHtml = ejs.render(template.content, variables);

    const styleContent = template.styles?.map((style) => style.content).join('\n') ?? '';
  ...
  async sendTemplate<T>(
    ...
    const html = await this.renderTemplateFromDb(templateName, variables as Record<string, string>);
