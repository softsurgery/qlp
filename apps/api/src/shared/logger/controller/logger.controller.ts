...
export class LoggerController {
  ...
  async findAll(@Query() options: IQueryObject): Promise<ResponseLogDto[]> {
    return toDtoArray(ResponseLogDto, await this.loggerService.findAll(options));
  }

  @Get('/list')
  @ApiPaginatedResponse(LogEntity)
  async findAllPaginated(@Query() query: IQueryObject): Promise<PageDto<ResponseLogDto>> {
