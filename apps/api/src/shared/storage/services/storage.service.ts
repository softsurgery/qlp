...
export abstract class StorageService {
  ...
  async findBySystematicName(systematicName: string): Promise<StorageEntity | null> {
  ...
  async findAll(query: IQueryObject = {}): Promise<StorageEntity[]> {
    const queryBuilder = new QueryBuilder(this.storageRepository.getMetadata());
    const queryOptions = queryBuilder.build(query);
    return await this.storageRepository.findAll(queryOptions as FindManyOptions<StorageEntity>);
  }
  ...
  abstract loadResource(slug: string, start?: number, end?: number): Promise<ReadStream>;
