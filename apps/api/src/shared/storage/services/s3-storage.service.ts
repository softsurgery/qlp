...
export class MinioStorageService extends StorageService {
  ...
  async loadResource(slug: string, start?: number, end?: number): Promise<ReadStream> {
    ...
    try {
      ...
      const stream = await this.minio.getObject(this.bucket, upload.relativePath);
      return stream as unknown as ReadStream;
    } catch (error) {
      throw new FileNotFoundException(error as Error);
    }
  }
  ...
  async store(
    ...
    try {
      ...
      await this.minio.putObject(this.bucket, key, this.bufferToStream(file.buffer), size, {
  ...
  async delete(id: number): Promise<StorageEntity> {
    ...
    try {
      ...
      throw new StorageBadRequestException(`Failed to delete file: ${upload.slug} ${error}`);
    }
  }
  ...
  async storeMultipleFiles(
    ...
    return Promise.all(
      files.map((file) => this.store(file, isTemporary, isPrivate, undefined, folderId)),
    );
  }
  ...
  async duplicate(id: number): Promise<StorageEntity> {
    ...
    try {
      const originalStream = await this.minio.getObject(this.bucket, original.relativePath);
      const chunks: Buffer[] = [];
      for await (const chunk of originalStream) chunks.push(chunk);
      const buffer = Buffer.concat(chunks);

      await this.minio.putObject(this.bucket, newKey, this.bufferToStream(buffer), buffer.length, {
